...
function initParameters(){
	...
	ProjectionMatrix=mat4();//单位矩阵
        ...
        // 雾化效果参数
    fogDensity = 0.05;    // 雾密度
    fogGradient = 20.0;   // 雾梯度（用于线性雾）
    fogColor = vec3(0.7, 0.7, 0.8); // 雾颜色（浅蓝色）
    fogType = 1;         // 雾类型：0=线性，1=指数，2=指数平方
...
function render(){	
    ...
    gl.uniform1f(gl.getUniformLocation(program, "fogGradient"), fogGradient);
...
window.onkeydown = function(e){
    ...
    switch (code) {
        ...
        case 71: // G键 - 减少当前类型雾的密度/梯度
            if (fogType === 0) {
                // 线性雾：减少梯度
                fogGradient = Math.max(fogGradient - 1.0, 1.0);
            } else {
                // 指数雾和指数平方雾：减少密度
                fogDensity = Math.max(fogDensity - 0.01, 0.0);
            }
            break;
        case 72: // H键 - 切换雾类型
            fogType = (fogType + 1) % 3;
            break;
case 84: // T键 - 切换雾颜色
    if (fogColor[0] === 0.7 && fogColor[1] === 0.7 && fogColor[2] === 0.8) {
        fogColor = vec3(0.3, 0.3, 0.3); // 灰色
    } else if (fogColor[0] === 0.3 && fogColor[1] === 0.3 && fogColor[2] === 0.3) {
        fogColor = vec3(0.9, 0.9, 0.7); // 浅黄色
    } else {
        fogColor = vec3(0.7, 0.7, 0.8); // 浅蓝色
    }
    break;
